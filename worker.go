...
func (w *Worker) Run(listenAddr string, listenPort int, done <-chan struct{}) error {
	...
	// Egress socket: unconnected UDP6, bound to an ephemeral port.
	...
	// Bind the egress socket to the configured NIC so multicast datagrams
	// exit on the correct interface.
	if err := udpEgress.SetMulticastInterface(w.iface); err != nil {
	...
	if err := udpEgress.SetMulticastLoopback(w.debug); err != nil {
